...
// The picture is the whole beam path, the emulator's `--full-raster`: 1024 samples by 312 lines, each line drawn twice "so the image stands at the proportions a screen had" [A], its own docs/command-line.en.md, which makes the image 1024 by 624 and its pixels square.
// Of that image the beam sweeps columns 192 to 991 and rows 48 to 619; everything outside is sync and blanking, which no phosphor sees [E], measured on the capture itself. The emulator's own smaller crop, "the window a monitor shows", is not that sweep: a real tube paints wider and taller, and shows more border than the crop holds.
